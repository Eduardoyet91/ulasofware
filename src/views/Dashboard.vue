...
<template>
  <div>
    ...
    <main class="relative -mt-48 mr-8 ml-8">
      <div class="grid grid-cols-2 px-2 flex justify-center lg:ml-6 lg:justify-center">
             ...
             <button @click="userStore.getProveebyQuery(query)"  class="flex mt-4 items-center justify-center px-4 py-2 border border-white text-base font-medium rounded-md shadow-sm text-white  bg-sky-600 hover:bg-sky-700 sm:px-8">Buscar </button>
 </div>
              </div>
              <Cat/>
            </div>
      ...
      <List v-show="userStore.result"/>
      <Empty v-show="!userStore.result"/>

      <Loading v-show="userStore.loadingBig"/>
